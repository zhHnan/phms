...
public interface RoomMapper extends BaseMapper<Room> {
        ...
        @Select("<script>" +
                        ...
                        "COALESCE(room_stats.avg_score, 0) as room_avg_score, " +
                        "COALESCE(room_stats.review_count, 0) as room_review_count, " +
                        "COALESCE(hotel_stats.avg_score, 0) as hotel_avg_score, " +
                        "COALESCE(hotel_stats.review_count, 0) as hotel_review_count " +
                        "FROM biz_rooms r " +
                        "LEFT JOIN sys_hotels h ON r.hotel_id = h.id " +
                        "LEFT JOIN (" +
                        "  SELECT o.room_id, AVG(hr.score) as avg_score, COUNT(hr.id) as review_count " +
                        "  FROM biz_orders o " +
                        "  INNER JOIN biz_hotel_reviews hr ON hr.order_id = o.id AND hr.is_deleted = 0 " +
                        "  WHERE o.is_deleted = 0 " +
                        "  GROUP BY o.room_id" +
                        ") room_stats ON r.id = room_stats.room_id " +
                        "LEFT JOIN (" +
                        "  SELECT hotel_id, AVG(score) as avg_score, COUNT(*) as review_count " +
                        "  FROM biz_hotel_reviews " +
                        "  WHERE is_deleted = 0 " +
                        "  GROUP BY hotel_id" +
                        ") hotel_stats ON r.hotel_id = hotel_stats.hotel_id " +
                        "WHERE r.is_deleted = 0 " +
                        "<if test='hotelId != null'> AND r.hotel_id = #{hotelId} </if>" +
                        ...
                        "ORDER BY r.id DESC" +
                        "</script>")
        List<Room> selectAvailableRoomsWithHotel(
                        @Param("hotelId") Long hotelId,
